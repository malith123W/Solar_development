...
# Smart Load Balancing & Forecasting imports
from gridlabd_integration import GridLABDIntegration
from load_balancing import LoadBalancer
from load_forecasting import LoadForecaster
...
# Initialize Smart Grid processors
gridlabd_processor = GridLABDIntegration(use_temp_files=True)
...
# Transformer Load Analysis routes
@app.route('/api/transformer_load/upload', methods=['POST'])
def transformer_load_upload():
    """Upload transformer load CSV file"""
    if 'file' not in request.files:
        return jsonify({'error': 'No file provided'}), 400
    ...
    try:
        ...
        for col in df.columns:
            if 'KVA' in col.upper() and 'IMPORT' in col.upper():
                kva_col = col
                break
        
        # Look for KW column
        for col in df.columns:
            if 'KW' in col.upper() and 'IMPORT' in col.upper() and 'KWH' not in col.upper():
                kw_col = col
                break
        ...
        for col in df.columns:
            col_upper = col.upper()
            if ('VOLTAGE' in col_upper or 'V_L' in col_upper or 'VL' in col_upper) and 'KV' not in col_upper:
                if not any(x in col_upper for x in ['MAX', 'MIN', 'THD', 'UNBALANCE']):
                    voltage_cols.append(col)
        
        if not kva_col and not kw_col:
            return jsonify({'error': 'CSV must contain KVA or KW columns'}), 400
        
        # Create time column if DATE and TIME exist
        if 'DATE' in df.columns and 'TIME' in df.columns:
            try:
                combined_datetime = df['DATE'].astype(str) + ' ' + df['TIME'].astype(str)
                for date_format in ['%Y-%m-%d %H:%M:%S', '%d/%m/%Y %H:%M:%S', '%m/%d/%Y %H:%M:%S', 'mixed']:
                    try:
        ...
        session_data[session_id]['pq']['transformer_load'] = {
            'data': df.to_dict('records'),
        ...
        return jsonify({
            ...
            'has_kva': kva_col is not None,
            'has_kw': kw_col is not None,
...
@app.route('/api/transformer_load/analyze', methods=['POST'])
def transformer_load_analyze():
    """Analyze transformer load and identify overload conditions"""
    try:
        data = request.get_json()
        session_id = data.get('session_id', 'default')
        rated_capacity = data.get('rated_capacity')
        
        if session_id not in session_data or 'transformer_load' not in session_data[session_id]:
            return jsonify({'error': 'No transformer load data found. Upload file first.'}), 400
        
        if not rated_capacity or rated_capacity <= 0:
            return jsonify({'error': 'Valid transformer rated capacity (kVA) is required'}), 400
        
        # Get stored data
        load_data = session_data[session_id]['transformer_load']
        df = pd.DataFrame(load_data['data'])
        kva_col = load_data['kva_col']
        kw_col = load_data['kw_col']
        ...
        if kw_col:
            ...
            # For KW, we'll use the same capacity (assuming power factor ~1 for simplicity)
        ...
        if 'pq' not in session_data[session_id]:
            session_data[session_id]['pq'] = {}
        if 'transformer_load' not in session_data[session_id]['pq']:
            session_data[session_id]['pq']['transformer_load'] = {}
        session_data[session_id]['pq']['transformer_load']['analysis'] = analysis_results
        ...
        return jsonify({'error': f'Error analyzing transformer load: {str(e)}'}), 500

@app.route('/api/transformer_load/export_csv', methods=['POST'])
def transformer_load_export_csv():
    """Export transformer load analysis results as CSV"""
    try:
        data = request.get_json()
        session_id = data.get('session_id', 'default')
        
        if session_id not in session_data or 'transformer_load' not in session_data[session_id]:
            return jsonify({'error': 'No transformer load data found'}), 400
        
        if 'analysis' not in session_data[session_id]['transformer_load']:
        ...
        csv_buffer.write("Transformer Load Analysis Summary\n\n")
        csv_buffer.write(f"Rated Capacity,{analysis['rated_capacity_kva']} kVA\n")
        csv_buffer.write(f"Analysis Period,{analysis['time_range'].get('start', 'N/A')} to {analysis['time_range'].get('end', 'N/A')}\n")
        csv_buffer.write(f"Total Records,{analysis['time_range'].get('total_records', 0)}\n\n")
        
        if analysis['kva_analysis']:
            kva = analysis['kva_analysis']
            csv_buffer.write("KVA Analysis\n")
            csv_buffer.write(f"Max Load,{kva['max_load_kva']:.2f} kVA,{kva['max_load_pct']:.2f}%\n")
            csv_buffer.write(f"Avg Load,{kva['avg_load_kva']:.2f} kVA,{kva['avg_load_pct']:.2f}%\n")
            csv_buffer.write(f"Min Load,{kva['min_load_kva']:.2f} kVA,{kva['min_load_pct']:.2f}%\n")
            csv_buffer.write(f"Overload Count,{kva['overload_count']} records\n")
            csv_buffer.write(f"Overload Duration,{kva['overload_duration_hours']:.2f} hours\n")
            csv_buffer.write(f"Total Overload Events,{kva['total_overload_events']}\n\n")
            
            if kva['overload_events']:
                csv_buffer.write("Overload Events\n")
                csv_buffer.write("Start,End,Max Load (kVA),Max Load (%),Duration (records)\n")
                for event in kva['overload_events']:
                    csv_buffer.write(f"{event['start']},{event['end']},{event['max_load_kva']:.2f},{event['max_load_pct']:.2f},{event['duration_records']}\n")
                csv_buffer.write("\n")
        
        if analysis['kw_analysis']:
            kw = analysis['kw_analysis']
            csv_buffer.write("KW Analysis\n")
            csv_buffer.write(f"Max Load,{kw['max_load_kw']:.2f} kW,{kw['max_load_pct']:.2f}%\n")
            csv_buffer.write(f"Avg Load,{kw['avg_load_kw']:.2f} kW,{kw['avg_load_pct']:.2f}%\n")
            csv_buffer.write(f"Min Load,{kw['min_load_kw']:.2f} kW,{kw['min_load_pct']:.2f}%\n")
            csv_buffer.write(f"Overload Count,{kw['overload_count']} records\n")
            csv_buffer.write(f"Overload Duration,{kw['overload_duration_hours']:.2f} hours\n")
        ...
        return send_file(
            ...
            mimetype='text/csv',
            as_attachment=True,
            download_name='transformer_load_analysis.csv'
        )
        
    except Exception as e:
        return jsonify({'error': f'Error exporting CSV: {str(e)}'}), 500

@app.route('/api/transformer_load/export_pdf', methods=['POST'])
def transformer_load_export_pdf():
    """Export transformer load analysis results as PDF"""
    try:
        data = request.get_json()
        session_id = data.get('session_id', 'default')
        transformer_name = data.get('transformer_name', 'Transformer')
        ...
        # Generate PDF
        pdf_buffer = generate_transformer_load_pdf(analysis, transformer_name)
        
        return send_file(
            pdf_buffer,
            mimetype='application/pdf',
            as_attachment=True,
            download_name='transformer_load_analysis.pdf'
        )
        
    except Exception as e:
        return jsonify({'error': f'Error exporting PDF: {str(e)}'}), 500

@app.route('/api/smart-grid/export_pdf', methods=['POST'])
def smart_grid_export_pdf():
    """Export Smart Grid analysis results as PDF with feeder voltage graphs"""
    try:
        data = request.get_json()
        session_id = data.get('session_id', 'default')
        transformer_name = data.get('transformer_name', 'Transformer')
        
        if session_id not in session_data or 'analysis_results' not in session_data[session_id]:
            return jsonify({'error': 'No Smart Grid analysis results found'}), 400
        
        analysis_results = session_data[session_id]['analysis_results']
        
        # Generate Smart Grid PDF with feeder voltage graphs
        pdf_buffer = generate_smart_grid_pdf(analysis_results, transformer_name)
        
        return send_file(
            pdf_buffer,
            mimetype='application/pdf',
            as_attachment=True,
            download_name='smart_grid_analysis_report.pdf'
        )
        
    except Exception as e:
        return jsonify({'error': f'Error exporting Smart Grid PDF: {str(e)}'}), 500
